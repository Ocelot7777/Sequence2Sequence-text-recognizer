...
class AsterBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None):
        ...
        if groups != 1 or base_width != 64:
            raise ValueError('This block only supports groups=1 and base_width=64')
        if dilation > 1:
            raise NotImplementedError('Dilation > 1 not supported in this block')
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer5 = self._make_layer(block, 512, layers[3], stride=(2, 1),
                                       dilate=replace_stride_with_dilation[2])
...
def recog_net():
    layers = [3, 4, 6, 3]
    model = ResNet(AsterBlock, layers)
    return model
